...
const Useracc = () => {
    ...
    const[useacclist, setuseacclist]  = useState([]);
    ...
    const totalPages = Math.ceil(useacclist.length / rowsPerPage);
    ...
    const getuseracc = async() => {
        ...
        const filter = res.filter(res => {
            const fil = usesearch === "" || res.usernum.toLowerCase().includes(usesearch.toLowerCase()) 
                                        || res.mail.toLowerCase().includes(usesearch.toLowerCase()) 
            return(fil)
        })
        setuseacclist(filter.reverse());
    };
    ...
    const creatacc = async () => {
        await fetch(`${backendbaseurl}/user`, {
            ...
            body : JSON.stringify({ usernum : number, pass : password, mail : email })
        });
        getuseracc();
    ...
    const updateacc = async() => {
        await fetch(`${backendbaseurl}/user/${up._id}`,{
            method : "PUT",
            headers : {
                "content-type" : "application/json"
            },
    ...
    const deleteacc = async (id) => {
        await fetch(`${backendbaseurl}/user/${id}`,{
            method : "DELETE",
        });
        getuseracc();
        setcreatefirst(false);
    ...
    const upd = (eee) => {
        setup(eee);
        setnumber(eee.usernum);
        setpassword(eee.pass);
        setemail(eee.mail);
        setupdatefirst(true);
        setshowmodel(true)
    }
    ...
    const exportee = () => {
        if (useacclist.length === 0) {
            alert("No data to export!");
            return;
        }

        const worksheet = XLSX.utils.json_to_sheet(useacclist);
    ...
    const currentRows = useacclist.slice(indexOfFirstRow, indexOfLastRow);
    ...
    return(
        ...
        <div>
            <div className="header">
                <h3 className="text-white text-3xl">Login Account</h3>
            </div>
            
            <div className="secheader">
                <div className="searchout">
                    <input className="searchinner" value={usesearch} onChange={(e) => setusesearch(e.target.value)} placeholder="Search by mobile and mail id"/>
                    <svg 
                                width="24" 
                                height="24" 
                                viewBox="0 0 24 24" 
                ...
                <div className="flex w-1/4 justify-between">
                    ...
                    </select>
                    {
                        updatefirst? (
                            ...
                            <input value={email} onChange={(wsd) => setemail(wsd.target.value)}/>
                            ...
                            <input value={password} onChange={(e) => setpassword(e.target.value)} />
                            <input value={email} onChange={(wsd) => setemail(wsd.target.value)}/>
                            <button onClick={creatacc}>Add</button>
                            <button onClick={cancel}>Cancel</button>
                            </>

                        ): <button className="addbut" onClick={crprior}>
                                Add
            ...
            <table className="tableout">
                <thead className="tablebgcolor">
                    <tr className="py-14  h-16">
                        ...
                        <th className="tableheadcol" >Password</th>
                ...
                <tbody>
                    {
                        ...
                        <tr key={acc._id} className={`py-14 h-20  ${index % 2 === 0 ? 'bg-white': 'bg-gray-100'} hover:bg-gray-200 `}>
                            <td className="tableheadcol" >{index+1}</td>
                            <td className="tableheadcol" >{acc.usernum}</td>
                            <td className="tableheadcol" > {acc.pass}</td>
                            <td className="tableheadcol" >{acc.mail}</td>
                            <td className="tableheadcol" >
                                <button className="updateview" onClick={() => upd(acc)}>
                                    <svg width="25px" height="25px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <button className="rowdel" onClick={() => deletemodel(acc._id)}>
                                    <svg width="25px" height="25px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <Modal show={showmodel} onClose={() => setshowmodel(false)} className="w-1/2  mx-auto ">
                ...
                <Modal.Body >
                    <input className='modalinput' placeholder="phone number" value={number} onChange={(e) => setnumber(e.target.value)}/>
                    <input className='modalinput' placeholder="password" value={password} onChange={(e) => setpassword(e.target.value)} />
                    <input className='modalinput' placeholder="mail id"  value={email} onChange={(wsd) => setemail(wsd.target.value)}/>
                
                </Modal.Body>
                <Modal.Footer className="footermd">
                
                <Button className="cancelmodal " onClick={(cancel)}>Cancel</Button>
                <Button className="addmodal" onClick={updatefirst? updateacc: creatacc}>{updatefirst? "Update" : "Add"}</Button>
                </Modal.Footer>
                </div>
            </Modal>
              ...
              <Modal show={showdelmodel} className="w-1/3  mx-auto ">
                ...
                <Button className="addmodal" onClick={()=> deleteacc(deleteid)}>Yes.. Do it</Button>
                </Modal.Footer>
                </div>
            </Modal>
        </div>
